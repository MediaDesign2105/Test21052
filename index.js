...
app.post('/create', function (req, res, next) {
  client.connect(err => {
    const customers = client.db("test21052").collection("customers");
...
app.get('/get-client', function (req, res) {
    client.connect(err => {
        client.db("test21052").collection("customers").findOne({name: req.query.name}, function(err, result) {
          if (err) throw err;
...
app.post('/update', function(req, res) {
  client.connect(err => {
    ...
    client.db("test21052").collection("customers").updateOne(query, newvalues, function(err, result) {
        if (err) throw err;
...
app.post('/delete', function(req, res) {
  client.connect(err => {
    ...
    client.db("test21052").collection("customers").deleteOne(query, function(err, obj) {
      if (err) throw err;
...
const uri = `mongodb+srv://${mongo_username}:${mongo_password}@cluster0.p5xrx8v.mongodb.net/?retryWrites=true&w=majority`; // change cluster url to your own
